...
Первый - проверяем находится ли товар уже в корзине (если да - генерирует событие на удаление карточки из корзины, с передачей в нем id).
Второй - проверяем на null цену продукта, для того чтобы опередлить бесценный ли товар.\
Поля класса:
...
В качестве методов имеет только сеттеры:
- `set text(value: string) ` - записывает в соответствующий элемент дескрипшн.
...
кнопки удаления из корзины (вешает на нее слушатель клика с генерацией события для презентера). Конструктор, помимо принятия привычных html контейнера
