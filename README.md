...
#### Класс Basket
Класс наследник Component, для отображения корзины. В конструктор прнимает экземпляр `EventEmitter` для инициации событий, а также записывает перечень карточек, общую стоимость и кнопку для\ 
...
#### Класс OrderInfo
Отвечает за отрисовку первой формы, а также за логику работы кнопки типа оплаты. Наследуются от Form, в конструктор получает клон темплейта а также экземпляр `EventEmitter`.
